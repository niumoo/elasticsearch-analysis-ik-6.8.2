    ...
    private boolean processEnglishLetter(AnalyzeContext context) {
        ...
        if (this.englishStart == -1) {// 当前的分词器尚未开始处理英文字符
            ...
            if (CharacterUtil.CHAR_ENGLISH == context.getCurrentCharType()) {
                ...
                // 遇到非English字符,输出词元
                Lexeme newLexeme = new Lexeme(context.getBufferOffset(), this.englishStart, this.englishEnd - this.englishStart + 1,
                    Lexeme.TYPE_ENGLISH);
                context.addLexeme(newLexeme);
                this.englishStart = -1;
                this.englishEnd = -1;
        ...
        if (context.isBufferConsumed() && (this.englishStart != -1 && this.englishEnd != -1)) {
            // 缓冲以读完，输出词元
            Lexeme newLexeme =
                new Lexeme(context.getBufferOffset(), this.englishStart, this.englishEnd - this.englishStart + 1, Lexeme.TYPE_ENGLISH);
            context.addLexeme(newLexeme);
            this.englishStart = -1;
            this.englishEnd = -1;
        }
    ...
    private boolean processArabicLetter(AnalyzeContext context) {
        ...
        if (this.arabicStart == -1) {// 当前的分词器尚未开始处理数字字符
            ...
            if (CharacterUtil.CHAR_ARABIC == context.getCurrentCharType()) {
                ...
                //// 遇到非Arabic字符,输出词元
                Lexeme newLexeme =
                    new Lexeme(context.getBufferOffset(), this.arabicStart, this.arabicEnd - this.arabicStart + 1, Lexeme.TYPE_ARABIC);
                context.addLexeme(newLexeme);
                this.arabicStart = -1;
                this.arabicEnd = -1;
            }
